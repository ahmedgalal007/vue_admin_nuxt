...
class Storage {
  ...
  _initState() {
    ...
    if (this._useVuex) {
      ...
      console.warn(
        '[AUTH] The Vuex Store is not activated. This might cause issues in auth module behavior, like redirects not working properly.To activate it, see https://nuxtjs.org/docs/2.x/directory-structure/store'
      )
    }
  }
  ...
  setCookie(key, value, options = {}) {
    ...
    const serializedCookie = cookie__default.default.serialize(
      _key,
      _value,
      _options
    )
...
const ConfigurationDocumentWarning = (message) =>
...
class ConfigurationDocument {
  ...
  async request() {
    ...
    const serverDoc =
      (_d =
        (_c =
          (_b = (_a = this.scheme.$auth.ctx) == null ? void 0 : _a.nuxtState) ==
          null
            ? void 0
            : _b.$auth) == null
          ? void 0
          : _c.openIDConnect) == null
        ? void 0
        : _d.configurationDocument
  ...
  setSchemeEndpoints() {
    ...
    this.scheme.options.endpoints = defu__default.default(
...
class RefreshToken {
  ...
  _updateExpiration(refreshToken) {
    ...
    const _tokenTTLMillis =
      Number(this.scheme.options.refreshToken.maxAge) * 1e3
    const _tokenExpiresAtMillis = _tokenTTLMillis
      ? _tokenIssuedAtMillis + _tokenTTLMillis
      : 0
    try {
      refreshTokenExpiration =
        jwtDecode__default.default(refreshToken + '').exp * 1e3 ||
...
class Token {
  ...
  _updateExpiration(token) {
    ...
    try {
      tokenExpiration =
        jwtDecode__default.default(token + '').exp * 1e3 ||
        _tokenExpiresAtMillis
    } catch (error) {
...
class IdToken {
  ...
  _updateExpiration(idToken) {
    ...
    try {
      idTokenExpiration =
        jwtDecode__default.default(idToken + '').exp * 1e3 ||
        _tokenExpiresAtMillis
    } catch (error) {
...
class Oauth2Scheme extends BaseScheme {
  ...
  get redirectURI() {
    ...
    return (
      ...
      urlJoin(requrl__default.default(this.req), path)
    )
  }
  ...
  get logoutRedirectURI() {
    return (
      ...
      urlJoin(
        requrl__default.default(this.req),
        this.$auth.options.redirect.logout
      )
    )
  }
